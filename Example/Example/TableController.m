...
- (void)viewDidLoad {
    ...
    if (self.flag==2) {
        ...
        NSLog(@"%@",self.pinYinData);
        
        self.totalData=[SortedProperty sortedModelDateWithArray:self.data PinYinData:self.pinYinData PropertyName:@"month"];
    }
    if (self.flag==3) {
      self.pinYinData=[SortedProperty sortedTitleStringWithArray:self.data PropertyName:@"title" IsBoolPropertyName:@"rating" FirstTitle:@"热"];
      self.totalData=[SortedProperty sortedModelStringWithArray:self.data PinYinData:self.pinYinData PropertyName:@"title" IsBoolPropertyName:@"rating"];
    }
    if (self.flag==4) {
